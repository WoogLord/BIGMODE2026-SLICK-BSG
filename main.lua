...
function love.update(dt)
    ...
    if inClubExpoFadeTimer >= 0 then
        inClubExpoFadeTimer = inClubExpoFadeTimer + (dt * 4)
    else
        inClubExpoFadeTimer = inClubExpoFadeTimer - (dt * 4)
    end
...
function love.draw()
    ...
    if isDebug then love.graphics.print("ONE NIGHT WINDOW - "..buildVersion.." - build time: 05-11-2026 at 05:15p ET", 5 * gfxScale, currWinDim.h-(gfxScale)-debugFont:getHeight(debugFont)) end
end